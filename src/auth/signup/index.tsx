...
import { useGithubAuth, useSignup } from "@/features/auth/hooks/useAuth";
  ...
  const onSubmit = (data: SignupPayload) => {
    mutate(data, {
      onSuccess: async (response) => {
        const isSuccessfulSignup = response?.status === "success";
    ...
    <section className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="rounded-xl border border-slate-200 p-8 backdrop-blur-sm bg-login-form-gradient">
          <h1 className="text-3xl font-bold mb-2 bg-linear-to-r from-slate-950 via-slate-800 to-slate-950 bg-clip-text text-transparent">Get started</h1>
          <p className="text-slate-600 mb-6">Create your account to manage deployments</p>

          {errors.name?.message && <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-lg text-sm">{errors.name?.message}</div>}
          {errors.username?.message && <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-lg text-sm">{errors.username?.message}</div>}
          ...
          <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div>
              ...
              <input type="text" {...register("name")} className="w-full px-4 py-2 rounded-lg border bg-white text-slate-950 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-0 transition-all" placeholder="John Doe" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Username</label>
              <input type="text" {...register("username")} className="w-full px-4 py-2 rounded-lg border bg-white text-slate-950 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-0 transition-all" placeholder="john.doe1" required />
            </div>
            <div>
            ...
            <Button disabled={isPending} className="w-full bg-accent text-text-primary text-base font-bold h-12 px-8 rounded-md hover:bg-[#e5a020] transition-all shadow-lg shadow-orange-100 hover:shadow-xl flex items-center justify-center gap-2 cursor-pointer">
              {isPending ? <Loading className="animate-spin" /> : "Create account"}
            </Button>
          </form>
          ...
          <p className="mt-6 text-center text-sm text-slate-600">
            ...
            <Link to="/login" className="font-medium hover:underline" style={{ color: "rgb(255, 178, 36)" }}>
              Sign in
            </Link>
          </p>
        </div>
