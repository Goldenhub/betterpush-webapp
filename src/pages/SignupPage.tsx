...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    // Signup functionality disabled for now
    console.log("Signup attempt:", { name, email, password });
  };

  return (
    <section className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      <div className="absolute inset-0 opacity-30" style={{ backgroundImage: "radial-gradient(circle at 80% 20%, rgba(255, 178, 36, 0.3), transparent 50%), radial-gradient(circle at 20% 80%, rgba(255, 178, 36, 0.2), transparent 50%)" }} />
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="rounded-xl border border-slate-200 p-8 backdrop-blur-sm" style={{ backgroundImage: "linear-gradient(135deg, rgba(255, 255, 255, 0.9), rgba(255, 178, 36, 0.05))" }}>
          <h1 className="text-3xl font-bold mb-2 bg-gradient-to-r from-slate-950 via-slate-800 to-slate-950 bg-clip-text text-transparent">Get started</h1>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-2 rounded-lg border bg-white text-slate-950 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-0 transition-all" style={{ borderColor: name ? "rgb(255, 178, 36)" : "rgb(226, 232, 240)", "--tw-ring-color": "rgb(255, 178, 36)" } as any} placeholder="John Doe" required />
            </div>
            <div>
